...
@router.get("/manifest")
def agent_tool_manifest(actor: Actor = Depends(get_current_actor)) -> dict:
...
def call_agent_tool(
    ...
    result = execute_tool(tool_name, payload, actor)
    response = {"tool": tool_name, "actor": actor.model_dump(), **result}
    if idempotency_key:
        store_response(idempotency_key, response)
    return response
...
def call_agent_tools_batch(
    ...
    for item in payload:
        ...
        result = execute_tool(tool_name, tool_payload, actor)
