    ...
    <div className="space-y-4">
      ...
      <section className="panel p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs uppercase tracking-[0.16em] text-slate-500">Kanban Operations</p>
            <h3 className="mt-1 text-xl font-semibold text-slate-900">Task Workflow Board</h3>
          </div>
          <select
        ...
        <section className="panel p-4">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
            {TASK_STATUSES.map((status) => (
              <article key={status} className="rounded-xl border border-slate-200 bg-slate-50/70 p-3">
                <div className="flex items-center justify-between">
                  <p className="text-xs uppercase tracking-[0.12em] text-slate-500">{TASK_STATUS_LABELS[status]}</p>
                    ...
                    <button
                      ...
                      className={[
                        ...
                        task.id === selectedTaskId
                          ...
                          : `border-slate-200 bg-white text-slate-800 ${TASK_STATUS_STYLES[task.status as TaskStatus]}`
                      ].join(" ")}
                    >
                      <p className="text-sm font-medium">{task.title}</p>
                      <p className="mt-1 text-xs text-slate-500">{task.priority.toUpperCase()}</p>
                    </button>
                  ))}
                </div>
              </article>
          ...
          {selectedTask ? (
            <div className="mt-2 space-y-3 text-sm">
              <div className="rounded-lg border border-slate-200 bg-white p-3">
                ...
                <p className="mt-2 text-xs text-slate-500">Assigned: {selectedTask.assigned_to ?? "Unassigned"}</p>
              </div>

              <div className="grid grid-cols-2 gap-2 text-xs">
                <button
                  type="button"
                  className="rounded-lg border border-teal-300 bg-teal-50 px-2 py-2 text-teal-700 disabled:opacity-50"
                ...
                <button
                  type="button"
                  className="rounded-lg border border-amber-300 bg-amber-50 px-2 py-2 text-amber-700 disabled:opacity-50"
                  disabled={actionState === "running"}
                ...
                <button
                  type="button"
                  className="rounded-lg border border-rose-300 bg-rose-50 px-2 py-2 text-rose-700 disabled:opacity-50"
                  disabled={actionState === "running"}
                ...
                <button
                  type="button"
                  className="rounded-lg border border-emerald-300 bg-emerald-50 px-2 py-2 text-emerald-700 disabled:opacity-50"
                  disabled={actionState === "running"}
              ...
              <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
                <p className="text-xs uppercase tracking-[0.12em] text-slate-500">Context</p>
                <pre className="soft-scroll mt-2 max-h-60 overflow-auto text-xs text-slate-700">
